...
<div class="container">
    ...
    <div style="position: absolute;margin-left: 980px;margin-top: -40px;">
        <c:if test="${empty user}">
            <a name="tj_login" class="lb" href="${ctx}/login.jsp" style="color: black">[登录]</a>
            &nbsp;&nbsp;
            <a name="tj_login" class="lb" href="${ctx}/register.jsp" style="color: black">[注册]</a>
        </c:if>
        <c:if test="${not empty user}">
            ...
            &nbsp;&nbsp;
            <a name="tj_login" class="lb" href="${ctx}/login/out" style="color: black">[退出]</a>
        </c:if>
    ...
    <nav class="navbar navbar-inverse">
        ...
        <div class="navbar-header">
            ...
            <a class="navbar-brand" href="${ctx}/index.jsp">首页</a>
        </div>
        <div id="navbar-menu" class="collapse navbar-collapse">
            <ul class="nav navbar-nav">
                ...
                <li><a href="#">博客问答</a></li>
                <li><a href="#">我的博客</a></li>
                <li><a href="${ctx}/personal/list?id=${user.id}">个人空间</a></li>
            </ul>
        </div>

        <form method="post" action="${ctx}/page/pages"  id="indexSearchForm"  class="navbar-form navbar-right" role="search" style="margin-top: -35px;margin-right: 10px">
        ...
        <div class="col-md-9"  style="background-color: white;">
                ...
                <c:forEach var="cont" items="${page.list}" varStatus="i">
                    ...
                    <div class="content-text">
                        ...
                        <div class="author clearfix">
                            <div>
                                <a href="${ctx}/other/list?oid=${cont.user.id}" target="_blank" rel="nofollow" style="height: 35px">
                                     <img src="${cont.user.imgUrl}">
                                </a>
                            </div>
                            <a href="${ctx}/other/list?oid=${cont.user.id}" target="_blank">
                                <h2 class="author-h2">
                                        ${cont.user.nickName} ${cont.formatDate}
                                </h2>
                            </a>
                        </div>
            ...
            <div id="page-info" style="position: absolute;width:900px;background-color: #EBEBEB;height: 80px;left: 0px;">
                <ul class="pager pager-loose">
                    ...
                    <c:if test="${page.pageNum > 1}">
                        <li class="previous"><a href="${ctx}/page/pages?pageNum=${page.pageNum-1}&&id=${user.id}&&keyword=${keyword}">« 上一页</a></li>
                    </c:if>
                    <c:forEach begin="1" end="${page.pages}" var="pn">
                        ...
                        <c:if test="${page.pageNum!=pn}">
                            <li ><a href="${ctx}/page/pages?pageNum=${pn}&&id=${user.id}&&keyword=${keyword}">${pn}</a></li>
                        </c:if>
                    </c:forEach>
                    ...
                    <c:if test="${page.pageNum<page.pages}">
                        <li><a href="${ctx}/page/pages?pageNum=${page.pageNum+1}&&id=${user.id}&&keyword=${keyword}">下一页 »</a></li>
                    </c:if>
                ...
                <div class="foot-nav clearfix" style="position: absolute;left: 0px;margin-top: 40px;text-align: center">
                    <div class="foot-copyrights" style="margin-left: 200px">
                        ...
                        <p>
                            ...
                            <span>邮箱：784503325@qq.com</span>
                        </p>
                        <p style="margin-top: 8px">&copy;caayb博客网版权所有</p>
                    </div>
                </div>
            </div>
            <!--col-md-9结束 -->
...
<script language=javascript>
    ...
    function personal(uId) {
        this.location =  "${ctx}/personal/list";
    }
    //日期格式化
    ...
    function upvote_click(id,cont) {
        ...
        $.ajax({
            type:'post',
            url:'${ctx}/page/up',
            data: {"contentId":id,"uid":'${user.id}',"upvote":cont},
            dataType:'json',
        ...
        if(showdiv_display = document.getElementById('comment_reply_'+id).style.display=='none'){//如果show是隐藏的
            ...
            $.ajax({
                ...
                success:function(data){
                    var list =  data.list;
                    var okHtml;  //总评论
                    if(list!=null && list!=""){     //如果评论集合list不为空遍历评论集合
                        $(list).each(function () {
                            var chtml = "<div class='comment-show'>"+           //一级评论模块
                            ...
                            okHtml = chtml;  //遍历一次到这里父评论先放入总评论中
                            //alert(this.children)
                            if(this.children != null && this.children != ''){ //开始遍历本次父评论自带的子平轮
                                ...
                                $(commentList).each(function () {
                                    ...
                                    okHtml = okHtml + oHtml;  //遍历一轮一级评论的结果就是一级评论+子评论
                                });


                            }

                            okHtml = okHtml+ehtml; //最终结果
    ...
    function comment_pl(content_id,comment_id,fid,by_id,cuid) {
        ...
        if(oHfVal.replace(/^ +| +$/g,'') == '' || oHfVal == oAllVal){
            ...
            $.ajax({
                type:'post',
                url:'${ctx}/page/commentChild',
                data: {"content_id":content_id,"uid":'${user.id}',"oSize":oHfVal,"comment_time":now,"by_id":by_id,"fid":fid},
                dataType:'json',
                success:function(data){
                    var comm_data =  data["data"];
                    //alert(comm_data);
