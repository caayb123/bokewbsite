...
<body>
<div class="alert alert-warning alert-dismissable" id="awr" style="display: none">
    <button type="button" id="closex" class="close">x</button>
    <p>您的评论有了新的回复,请点击导航栏的消息查看</p>
</div>
<nav class="navbar navbar-inverse" role="navigation">
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse navbar-collapse-example">
            ...
            <ul class="nav navbar-nav">
                <li><a href="${ctx}/personal/msgList" id="msg1">消息</a></li>
            </ul>
    ...
    <div class="designer-card card-media">
        ...
        <div class="author-info">
            ...
            <div style="float: left;margin-top: 40px; margin-left: 130px;">
                <a href="${ctx}/personal/profile?id=${user.id}"><i class="icon icon-edit"></i><span style="margin-left: 10px">修改个人资料</span></a>
            </div>
        </div>
    </div>
    ...
    <div class="dreamland-content" style="background-color: white">
        ...
        <div id="update-dreamland" style="height: 700px;margin-top: 50px;width: 100%;display: none" >
            <ul style="font-size: 12px" id="update-dreamland-ul">
                ...
                <li class="dreamland-fix">
                    ...
                    <a href="${ctx}/jump/deleteContent?cid=${cont.id}"><span class="bar-delete">删除</span></a>
                    <a href="${ctx}/jump/writedream?cid=${cont.id}"><span class="bar-update">修改</span></a>
        ...
        <div id="personal-dreamland" style="height: 700px;margin-top: 50px;width: 100%;display: none">
            <ul style="font-size: 12px" id="personal-dreamland-ul">
                <c:forEach var="cont" items="${page2.list}" varStatus="i">
                <li class="dreamland-fix">
                    <a href="${ctx}/jump/watch?cid=${cont.id}">${cont.title}</a>
                    <a href="${ctx}/jump/deleteContent?cid=${cont.id}"><span class="bar-delete">删除</span></a>
                    <a href="${ctx}/jump/writedream?cid=${cont.id}"><span class="bar-update">修改</span></a>
        ...
        <div class="hot-dreamland" style="height: 1020px">
            ...
            <div style="height: 700px;margin-top: 30px;width: 100%" id="hot-dreamland">
                <ul style="font-size: 12px" id="hot-dreamland-ul">
                    <c:forEach var="cont" items="${hotPage.list}" varStatus="i">
                    <li class="dreamland-fix">
                        <a href="${ctx}/jump/watch?cid=${cont.id}">${cont.title}</a>
                        <span class="bar-read">评论 (${cont.commentNum} )</span>
                        <span class="bar-commend">${cont.upvote}人阅读</span>
...
<script>
    $(function () {
        ...
        $("#closex").click(function () {
            $("#awr").attr("style","display:none;");//隐藏div
        })
        //进入页面首先加载
        $.ajax({
            type: 'post',
            url: '${ctx}/personal/msg',
            dataType: 'json',
            success:function (data) {
                if (data.num=='no'){
                    ...
                    $("#awr").attr("style","display:block;");//显示div

                    $("#msg1").text("消息("+data.num+")")
                }
            }
        })
        //每隔10秒查询消息通知事件
   ...
   function changeToActive(id,category,pageNum) {
       ...
       $.ajax({
           type: 'post',
           url: '${ctx}/personal/findByCategory',
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(pageCate=="fail"){
                    ...
                    if(ucList!=null) {
                        $(ucList).each(function () {
                            var contHtml = " <li class='dreamland-fix'> <a href='${ctx}/jump/watch?cid="+this.id+"'>"+this.title+"</a><a href='${ctx}/jump/deleteContent?cid="+this.id+"'><span class='bar-delete'>删除</span></a>"
                                +"<a href='${ctx}/jump/writedream?cid="+this.id+"'><span class='bar-update'>修改</span></a><hr/></li>";
                            startHtml = startHtml + contHtml;
                        });
                        var okHtml = startHtml + endHtml;
    ...
    function hotTurnPage(pageNum) {
        ...
        $.ajax({
            type: 'post',
            url: '${ctx}/personal/findAllHotContents',
            data: {"pageNum": pageNum},
            dataType: 'json',
            success: function (data) {
                var pageCate = data["hotPage"];
                console.log(pageCate.list)
                if(pageCate=="fail"){
                    window.location.href = "${ctx}/login.jsp";
                }else{
                    var ucList = pageCate.list;
